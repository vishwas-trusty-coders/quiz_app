...
class TimedQuizController extends Controller
{
    ...
    public function startQuiz($quizId)
    {
        ...
        return view('timed_quiz', compact('quiz', 'questions', 'questionTimes'));
    }
    ...
    public function attemptQuiz(Request $request, $quizId)
    {
        ...
        $quiz->questions()->updateExistingPivot($request->question_id, [
            'selected_answer' => !empty($request->selected_answer) ? $request->selected_answer: null,
            'is_attempted' => !empty($request->selected_answer)? true : false,
            'is_correct' => !empty($request->selected_answer) ? ($request->selected_answer == $request->correct_answer) : 0,
    ...
    public function endQuiz(Request $request, $quizId)
    {
        ...
        $quiz->remaining_time = $request->remaining_time;
        $quiz->save();
